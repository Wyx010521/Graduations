...
import com.wyx.springboot.mapper.UserMapper;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
       ...
       User one = getUserInfo(userDTO);
        if ( one != null){
           ...
           userDTO.setToken(token);
            ...
            throw new  ServiceException(Constants.CODE_666,"用户名或密码错误");
        }
    }
    ...
    public User register(UserDTO userDTO) {
        ...
        if ( one == null){
          ...
          save(one);//把copy完之后的用户 对象存到数据库
        }else {
            throw new  ServiceException(Constants.CODE_666,"用户已存在");
        }
      return one;
    }
